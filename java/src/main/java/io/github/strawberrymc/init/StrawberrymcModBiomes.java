...
@EventBusSubscriber
public class StrawberrymcModBiomes {
		...
		for (LevelStem levelStem : levelStemTypeRegistry.stream().toList()) {
			...
			if (dimensionType.is(BuiltinDimensionTypes.NETHER) || dimensionType.is(BuiltinDimensionTypes.OVERWORLD)) {
				if (levelStem.generator() instanceof NoiseBasedChunkGenerator noiseGenerator) {
					((StrawberrymcModNoiseGeneratorSettings) (Object) noiseGenerator.generatorSettings().value()).setstrawberrymcDimensionTypeReference(dimensionType);
				}
			}
		}
	}
	...
	public static <T> Climate.ParameterList<T> adaptPresetParameterList(Identifier idArg, Climate.ParameterList<T> originalList, Function<ResourceKey<Biome>, T> lookup) {
		...
		if (idArg.equals(OVERWORLD_BIOMESOURCE_PRESET_ID))
			return StrawberrymcModBiomes.modifyOverworldParameterPoints(originalList, lookup);
		return originalList;
	}
	...
	public interface StrawberrymcModNoiseGeneratorSettings {
		void setstrawberrymcDimensionTypeReference(Holder<DimensionType> dimensionType);
	}
}